...
#ifndef STRUCT_COMMAND_PARSING_H
# define STRUCT_COMMAND_PARSING_H
# define S_QUOTES 39
# define D_QUOTES 34
...
{
	char					*source_command;
	t_list					*parsed_command; // t_command_id
}							t_cmd_cont;

typedef struct				s_command_executor
{
	char					*value;
	void					*args;
	char					id;
...
t_cmd_cont \
		*command_parsing(char *command_line);
int		str_table_size(char **table);
char	**ft_insert_str(int place_for_insert, char **to_realloc, char *to_insert);
